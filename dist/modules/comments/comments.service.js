...
let CommentsService = class CommentsService {
    ...
    async commentsByMe(id) {
        try {
            const comments = await this.commentModel.find({ userId: id }).exec();
            if (comments.length === 0) {
                throw new common_1.HttpException('No comments found for the user', 404);
            }
            return comments;
        }
        catch (err) {
...
exports.CommentsService = CommentsService = __decorate([
    ...
    __param(0, (0, mongoose_1.InjectModel)(comments_schema_1.CommentEntity.name)),
